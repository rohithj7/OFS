    ...
    <>
      ...
      {!showPreviousOrders && (
        <div class="row">
              ...
              <table class="table-responsive-md table table-borderless border border-2 table-hover">
                ...
                {currentOrder.map((order) => (
                  <tr class="text-center" key={order.saleId}>
                    ...
                    <td class="py-4 align-middle">{new Date(order.saleDate).toLocaleDateString()}</td>
          ...
          <div class="h3 text-center mt-4 mb-3">Status<span class="py-4"><span class="ms-2 p-2 text-uppercase badge bg-pastelblue">{currentOrder ? renderStatusText(currentOrder.saleStatus) : "N/A"}</span></span></div>
          
          <div class="card m-1 border-2">
              <div class="card-header px-3 pt-4 border-0 bg-light">
              <p class="mt-1 mb-4 text-center fs-5">Shipping Address: <span class="py-4"><span class="">{shippingAddress || "Loading..."}</span></span></p>
              </div>
                ...
                <table class="table-responsive-md table table-borderless border border-2 table-hover">
                  <thead class="table-light">
                    <tr class="text-center">
                      ...
                      <th colspan="1"><div class="py-4 text-uppercase">Order Total</div></th>
                      <th colspan="1"><div class="py-4 text-uppercase">Date</div></th>
                      <th colspan="1"><div class="py-4 text-uppercase">Shipping Address</div></th>
                      <th colspan="1"><div class="py-4 text-uppercase">Order Status</div></th>
                      <th colspan="1"><div class="py-4 text-uppercase">View Order</div></th>
                  ...
                  <tbody>
                    ...
                    <tr class="text-center" key={order.saleId}>
                      ...
                      <td class="py-4 align-middle">${Number(order.totalPrice).toFixed(2)}</td>
                      <td class="py-4 align-middle">{new Date(order.saleDate).toLocaleDateString()}</td>
                      <td class="py-4 align-middle"><span class="small">Shipping Address: {shippingAddress || "Loading..."}</span></td>
                      <td class="py-4 align-middle"><span class="p-2 text-uppercase badge bg-green">{renderStatusText(order.saleStatus)}</span></td>
                      {/* <td class="py-4 align-middle"><span class="p-2 text-uppercase badge bg-green">Being prepared</span></td> */}
                      <td class="py-4 align-middle">
                      <Link
