...
app.get('/sales/:id', isAuthenticated, async (req, res) => {
  try {
...
app.post('/sales', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!customerId || !employeeId || !saleDate || !totalAmount) {
      return res.status(400).json({
        error: 'Required fields: customerId, employeeId, saleDate, and totalAmount'
      });
    }
...
app.get('/sales-products', isAuthenticated, async (req, res) => {
  try {
...
app.post('/sales-products', isAuthenticated, async (req, res) => {
  try {
    const { saleId, productId, quantity, price } = req.body;

    if (!saleId || !productId || !quantity || !price) {
      return res.status(400).json({
        error: 'Required fields: saleId, productId, quantity, and price'
      });
    }

    const newSalesProduct = await createSalesProduct({
      saleId,
      productId,
      quantity,
      price
...
app.post('/orders', isAuthenticated, async (req, res) => {
  try {
    const { supplierId, orderDate, totalAmount, status } = req.body;

    if (!supplierId || !orderDate || !totalAmount || !status) {
      return res.status(400).json({
        error: 'Required fields: supplierId, orderDate, totalAmount, and status'
      });
    }
...
// ORDERS_PRODUCTS routes
app.get('/orders-products', isAuthenticated, async (req, res) => {
  try {
    const ordersProducts = await getOrdersProducts()
    res.json(ordersProducts)
  } catch (err) {
...
app.post('/orders-products', isAuthenticated, async (req, res) => {
  try {
    const { orderId, productId, quantity, price } = req.body;

    if (!orderId || !productId || !quantity || !price) {
      return res.status(400).json({
        error: 'Required fields: orderId, productId, quantity, and price'
      });
    }
    ...
    console.error('Error creating order-product:', err);
    res.status(500).send(err.message);
  }
});
...
app.post('/balance', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!balance || !date) {
      return res.status(400).json({
        error: 'Required fields: balance and date'
      });
    }

    const newBalanceId = await storeBalance({
      balance,
      date
    });
